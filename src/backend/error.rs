...
//!
//! Backend modules and the MLIR boundary use one result type and distinct stage codes.
...
pub(crate) type BackendResult<T> = Result<T, Diagnostics>;

/// Construct one lowering-stage diagnostic list.
pub(crate) fn lowering_error(code: &'static str, message: impl Into<String>) -> Diagnostics {

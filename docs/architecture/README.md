...
Bedrock compiles typed programs to native code. Multi-Level Intermediate
...
> object emission for `i64` scalar host functions. Tensor lowering, the
> reference interpreter, ownership, effects, graphics processing unit (GPU)
> code generation, and self-hosting remain design contracts rather than
> implemented claims.
...
```mermaid
flowchart LR
    source["Bedrock source"] --> frontend["Frontend"]
    frontend --> hir["Typed HIR"]
    hir --> mlir["MLIR optimization"]
...
High-level intermediate representation (HIR) records Bedrock semantics after
name and type checking. MLIR then rewrites tensor operations into explicit
control flow and memory access. A planned small interpreter will define the
behavior of that restricted form. The backend translates the same form to
